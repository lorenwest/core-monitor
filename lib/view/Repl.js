// Repl.js (c) 2010-2013 Loren West and other contributors
...
(function(root){
  ...
  var Monitor = root.Monitor || require('monitor-dashboard'),
      ...
      STORAGE_KEY_CONSOLE = 'core.replConsole',
      STORAGE_KEY_MACROS = 'core.replMacros',
      _ = Monitor._;
  ...
  */
  var Repl = core.Repl = Backbone.View.extend({
    ...
    initialize: function(options) {
      ...
      if (!t.monitor.get('probeClass')) {
        t.monitor.set({
          probeClass: 'Repl',
          hostName: 'localhost',
          initParams: {uniqueInstance:1}
        });
      }

      // Assure we have a unique probe instance.
      t.monitor.get('initParams').uniqueInstnce = Math.random();
    },
    ...
    render: function() {
      ...
      t.body = $('<ul class="nm-core-rp"></ul>').appendTo(t.$el);
      t.input = $('<input class="nm-core-rp-input" type="text"/>')
        .appendTo(t.body)
        .keydown(function(e){
      ...
      t.suggestLine = $('<li class="nm-core-rp-suggest-line"></li>').appendTo(t.body);
      t.suggestLede = $('<span class="nm-core-rp-lede"></span>').appendTo(t.suggestLine);
      t.suggestTail = $('<span class="nm-core-rp-tail"></span>').appendTo(t.suggestLine);
      t.clearSuggestions();
      t.mergeHistory('');
    ...
    consoleOut: function(out){
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        // Generate the output
        lastLine = $('<li class="output_line"><span class="nm-core-rp-out"></span></li>')
          .appendTo(t.body);
        lastLine.find('.nm-core-rp-out').text(line);
      }
    ...
    resize: function(){
      var t = this, parent = t.input.parent(),
          promptWidth = $('.nm-core-rp-out', parent).width();
    ...
    xferInput: function(){
      ...
      $('<span class="nm-core-rp-entered"></span>')
        .text(value)
    ...
    editMacro: function(name) {
      ...
      if (!macro) {
        macro = new core.Macro({id:name});
        t.macros.add(macro);
      }

      // Open the macro editor
      var editor = new core.MacroEditor({
